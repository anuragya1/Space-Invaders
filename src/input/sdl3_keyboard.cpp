// sdl3_keyboard.cpp - implementation of SDL3Keyboard.
...
void SDL3Keyboard::note_key_down(SDL_Keycode k) {
    // SDL3 keycodes for letter / punctuation keys are their ASCII values.
    if (k == 0x20)                   pendingShoot_ = true;   // space
    else if (k == 0x70 || k == 0x50) pendingPause_ = true;   // p / P
    else if (k == 0x71 || k == 0x51) pendingQuit_  = true;   // q / Q
}
...
std::uint8_t SDL3Keyboard::poll(std::uint32_t /*tick*/, const Game& g,
    ...
    if (pendingShoot_) {
        m |= action::SHOOT;
        pendingShoot_ = false;
        // A deliberate tap resets the auto-fire cooldown so the player
        // gets immediate feedback during Rapid even when alternating
        // taps and holds.
    ...
    if (rapidActive && spaceHeld) {
        if (autofireCooldown_ <= 0 && !(m & action::SHOOT)) {
            // Only emit the auto-shot if a deliberate-tap shot wasn't
            // already going out this tick (avoids doubling up).
            m |= action::SHOOT;
            autofireCooldown_ = AUTOFIRE_COOLDOWN_TICKS;
        }
    }
